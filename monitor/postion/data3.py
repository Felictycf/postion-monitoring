...
"""
# 收盘价价格幅度和5分钟
Fast Binance UM Perpetual - Open Interest Anomaly Scanner (with 5m + price change)
- 并发扫描 USDT 永续（可切换全量），周期：5m / 15m / 30m / 1h / 4h
- 先仅抓 OI 历史做 z-score + 百分比，触发后再拉该标的该周期的最新两根收盘价，计算价格变化幅度
...
# 扫描 24h 成交额 Top N（None=不筛选；注意：5m 加入后总调用量会提高，如限频可适当降低 N）
TOP_N = 120
...
# 初始化 UMFutures（兼容老版本库：不传 session）
